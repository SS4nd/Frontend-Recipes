...
async function addCategory(newCategory: string): Promise<Array<string>> {
  ...
  if (!categories.includes(newCategory)) {
    ...
    // Make the API call to add the category to the server
    const options = makeOptions("POST", { "name": newCategory });
    await fetch(CATEGORIES_URL, options).then(handleHttpErrors);
  }
